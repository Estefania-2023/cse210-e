...
public abstract class Activity
{
    ...
    {
        _speed = Math.Round(CalculatePace(), 2);
    }
    ...
    {
        _pace = Math.Round(CalculateSpeed(), 2);
    }
    ...
    public virtual void GetSummary()
    {
        ...
        string summary = $"{dateFormat} {GetExerciseName()} ({GetExerciseDuration()} min):  Distance {GetCalculatedDistance()} {GetMetricUnit()}, Speed {GetCalculatedSpeed()} {GetMetricUnit()} per hour, Pace {GetCalculatedPace()}  min per {GetMetricUnit()}";
